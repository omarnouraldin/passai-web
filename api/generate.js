...
async function checkUsage(authHeader) {
  ...
  if (!token) return { isPro: false, userId: null, userClient: null };

  const supabaseUrl  = process.env.VITE_SUPABASE_URL;
  const supabaseAnon = process.env.VITE_SUPABASE_ANON_KEY;
  if (!supabaseUrl || !supabaseAnon) return { isPro: false, userId: null, userClient: null };

  try {
    const supabase = createClient(supabaseUrl, supabaseAnon);
    const { data: { user }, error } = await supabase.auth.getUser(token);
    if (error || !user) return { isPro: false, userId: null, userClient: null };
    ...
    if (e.status) throw e; // re-throw our limit error
    return { isPro: false, userId: null, userClient: null };
  }
}

// ── Increment generation counter after success ────────────────────────────────
async function incrementUsage(userClient, userId, used) {
  if (!userClient || !userId) return;
  try {
    await userClient.from('profiles')
