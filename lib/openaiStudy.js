...
function addSelectiveFuriganaToPlainText(text, existingTerms = new Set(), maxAdds = 4) {
  let result = text;
  let added = 0;
...
export function ensureSelectiveFurigana(text, maxAdds = 8) {
  ...
  const targetCount = Math.max(0, Math.min(maxAdds, Math.floor(length / 80) + 1));
...
function normalizeTextField(text) {
  return ensureSelectiveFurigana(normalizeGeneratedText(text));
}
